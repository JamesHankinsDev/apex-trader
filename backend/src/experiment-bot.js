...
class ExperimentBot {
  constructor() {
    this.running = false;
    this.config = {
      apiKey: process.env.EXPERIMENT_ALPACA_API_KEY || '',
      secretKey: process.env.EXPERIMENT_ALPACA_SECRET_KEY || '',
      mode: 'paper', // Always paper for experiments
      watchlist: (process.env.EXPERIMENT_WATCHLIST || 'BTC/USD,ETH/USD,SOL/USD').split(','),
      positionSize: parseFloat(process.env.EXPERIMENT_POSITION_SIZE) || 0.33,
      dipThreshold: parseFloat(process.env.EXPERIMENT_DIP_THRESHOLD) || 0.015,
      maxPositions: parseInt(process.env.EXPERIMENT_MAX_POSITIONS) || 2,
      scanInterval: parseInt(process.env.EXPERIMENT_SCAN_INTERVAL_SECONDS) || 30,
      maxHoldHours: parseInt(process.env.EXPERIMENT_MAX_HOLD_HOURS) || 4,
    };
