...
function EquityChart({ data, startValue }) {
  ...
  useEffect(() => {
    ...
    // Grid
    ctx.strokeStyle = 'rgba(26,26,46,0.8)'; ctx.lineWidth = 1;
    for (let i = 0; i <= 4; i++) {
      const y = (i / 4) * H;
      ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(W, y); ctx.stroke();
      ctx.fillStyle = '#444466'; ctx.font = '10px Share Tech Mono, monospace';
    ...
    if (startValue) {
      ...
      ctx.setLineDash([4, 4]); ctx.strokeStyle = 'rgba(68,136,255,0.35)'; ctx.lineWidth = 1;
...
export default function Dashboard() {
  ...
  const [config, setConfig] = useState({
    positionSize: 80, stopLoss: 8, takeProfit: 25,
    rsiBuy: 35, rsiSell: 70, scanInterval: 60,
  });
  ...
  return (
    <div style={{ position: 'relative', zIndex: 1 }}>
      ...
      <header className={styles.header}>
        <div className={styles.logo}>APEX<span>TRADER</span></div>
        <div className={styles.headerRight}>
          ...
          <div className={styles.pill}>
            ...
            <span style={{ color: mode === 'live' ? 'var(--red)' : 'var(--yellow)' }}>
              {mode.toUpperCase()}
            </span>
          </div>
      ...
      <div className={styles.statsBar}>
        {[
          ...
          { label: 'TOTAL TRADES', val: status?.totalTrades || 0, sub: `Win rate: ${status?.winRate != null ? status.winRate + '%' : '—'}`, color: 'var(--blue)' },
          { label: 'OPEN POSITIONS', val: Object.keys(status?.positions || {}).length, sub: `Last scan: ${fmtTime(status?.lastScan)}`, color: 'var(--yellow)' },
        ].map(s => (
      ...
      <div className={styles.grid}>
        ...
        <div className={styles.panel}>
          ...
          {(status?.signals || []).length === 0
            ...
            : (status.signals).map(s => (
              <div key={s.symbol} className={`${styles.signalCard} ${s.score >= 70 ? styles.hot : s.score >= 50 ? styles.warm : styles.cold}`}>
                <div className={styles.signalHeader}>
                  <span className={styles.ticker}>{s.symbol.replace('/USD', '')}</span>
                  <span className={`${styles.scoreTag} ${s.score >= 70 ? styles.scoreHigh : s.score >= 50 ? styles.scoreMed : styles.scoreLow}`}>
                    SCORE {s.score}
                  </span>
                </div>
                <div className={styles.metrics}>
                  {[
                    { l: 'PRICE', v: s.price < 1 ? s.price.toFixed(4) : s.price.toFixed(2), prefix: '$' },
                ...
                {s.reasons?.length > 0 && (
                  <div className={styles.reasons}>{s.reasons.slice(0, 2).join(' · ')}</div>
                )}
              </div>
            ))}
          ...
          {Object.keys(status?.positions || {}).length === 0
            ...
            : Object.values(status.positions).map(pos => {
              const pnlPct = ((pos.entryPrice ? (pos.entryPrice - pos.entryPrice) / pos.entryPrice * 100 : 0));
              return (
                <div key={pos.symbol} className={styles.posCard}>
                  <div className={styles.posHeader}>
                    <span className={styles.ticker}>{pos.symbol?.replace('/USD', '')}</span>
                    <span className={styles.posEntry}>Entry {fmt$(pos.entryPrice)}</span>
                  </div>
                  ...
                  <div className={styles.posDetails} style={{ marginTop: 2, color: 'var(--dim)' }}>
                    {fmtTime(pos.entryTime)} · ${pos.notional?.toFixed(2)} invested
                  </div>
                </div>
              );
            })}
        ...
        <div className={styles.center}>
          <div className={styles.chartArea}>
            ...
            <EquityChart data={status?.equityHistory || [{ t: Date.now(), v: 100 }]} startValue={sv} />
          </div>
        ...
        <div className={`${styles.panel} ${styles.rightPanel}`}>
          ...
          <div className={styles.configSection}>
            ...
            <input className={styles.apiInput} type="text" placeholder="API KEY ID"
              value={apiKey} onChange={e => setApiKey(e.target.value)} disabled={running} />
            <input className={styles.apiInput} type="password" placeholder="SECRET KEY"
              value={secretKey} onChange={e => setSecretKey(e.target.value)} disabled={running} />
            <div className={styles.hint}>
              Paper: paper-api.alpaca.markets<br />
          ...
          <div className={styles.configSection}>
            ...
            {[
              { key: 'positionSize', label: 'Position Size', min: 10, max: 95, suffix: '%' },
              { key: 'stopLoss', label: 'Stop Loss', min: 2, max: 25, suffix: '%', prefix: '-' },
              { key: 'takeProfit', label: 'Take Profit', min: 5, max: 100, suffix: '%', prefix: '+' },
              { key: 'rsiBuy', label: 'RSI Buy Below', min: 20, max: 50, suffix: '' },
              { key: 'rsiSell', label: 'RSI Sell Above', min: 55, max: 85, suffix: '' },
              { key: 'scanInterval', label: 'Scan Interval', min: 15, max: 300, step: 15, suffix: 's' },
            ].map(s => (
              <div key={s.key}>
                <div className={styles.sliderRow}>
                  <span className={styles.sliderLabel}>{s.label}</span>
                  <span className={styles.sliderVal}>{s.prefix || ''}{config[s.key]}{s.suffix}</span>
                </div>
                <input type="range" min={s.min} max={s.max} step={s.step || 1}
                  value={config[s.key]}
                  onChange={e => setConfig(c => ({ ...c, [s.key]: parseInt(e.target.value) }))}
                />
              </div>
            ))}
            <button className={styles.btnApply} onClick={handleConfigUpdate}>APPLY CONFIG</button>
          </div>

          <div className={styles.configSection}>
            {!running
              ? <button className={styles.btnStart} onClick={handleStart} disabled={connecting}>
                  {connecting ? 'CONNECTING...' : '▶ START BOT'}
                </button>
              : <button className={styles.btnStop} onClick={handleStop}>■ STOP BOT</button>
            }
          </div>
      ...
      <div className={styles.disclaimer}>
        ⚠ <span style={{ animation: 'blink 1s steps(1) infinite', display: 'inline-block' }}>RISK WARNING:</span>{' '}
